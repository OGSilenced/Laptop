import React from 'react';
import useStyles from './index.styles';
import { Typography } from '@mui/material';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faCog, faWifi } from '@fortawesome/free-solid-svg-icons';

import { BoostingIcon } from '../AppContainer/icons/BoostingIcon';
import { BennysIcon } from '../AppContainer/icons/BennysIcon'; 
import { HoeIcon } from '../AppContainer/icons/HOEIcon'; 
import { BluePlazaIcon } from '../AppContainer/icons/BluePlazaIcon'; 
import { OrterIcon } from '../AppContainer/icons/OrterIcon';
import { TowServiceIcon } from '../AppContainer/icons/TowServiceIcon' ;
import { UnknownIcon } from '../AppContainer/icons/UnknownIcon';
import { ANALyzerIcon } from '../AppContainer/icons/ANALyzerIcon' ;
import { DodoIcon } from '../AppContainer/icons/DodoIcon' ;

export const Taskbar: React.FC<{
    activeApps: any;
    setActiveApps: any;
    setMinimizedApps: any;
    minimizedApps: any;
}> = (props) => {
    const classes = useStyles();

    const Applications = [
    {
        'icon': <BoostingIcon className={classes.icon}></BoostingIcon>,
        'title': 'Boosting',
        'open': function () {
            props.setMinimizedApps(prevState => ({
                ...prevState,
                BoostingAppMinimized: false
              }));
        },
        'show': function () {
            return props.activeApps.showBoostingApp;
        },
        'opened': props.activeApps.showBoostingApp && !props.minimizedApps.BoostingAppMinimized,
    },
    {
        'icon': <BennysIcon className={classes.icon}></BennysIcon>,
        'title': 'Bennys Parts',
        'open': function () {
            props.setMinimizedApps(prevState => ({
                ...prevState,
                BennysAppMinimized: false
              }));
        },
        'show': function () {
            return props.activeApps.showBennysApp;
        },
        'opened': props.activeApps.showBennysApp && !props.minimizedApps.BennysAppMinimized,
    },
    {
        'icon': <HoeIcon className={classes.icon}></HoeIcon>,
        'title': 'HOE',
        'open': function () {
            props.setMinimizedApps(prevState => ({
                ...prevState,
                HOImportAppMinimized: false
              }));
        },
        'show': function () {
            return props.activeApps.showHOImportApp;
        },
        'opened': props.activeApps.showHOImportApp && !props.minimizedApps.HOImportAppMinimized,
    },
    {
        'icon': <BluePlazaIcon className={classes.icon}></BluePlazaIcon>,
        'title': 'Blue Plaza',
        'open': function () {
            props.setMinimizedApps(prevState => ({
                ...prevState,
                MethAppMinimized: false
              }));
        },
        'show': function () {
            return props.activeApps.showMethApp;
        },
        'opened': props.activeApps.showMethApp && !props.minimizedApps.MethAppMinimized,
    },
      {
        'icon': <OrterIcon className={classes.icon}></OrterIcon>,
        'title': 'Örter',
        'open': function () {
            props.setMinimizedApps(prevState => ({
                ...prevState,
                HerbsAppMinimized: false
              }));
        },
        'show': function () {
            return props.activeApps.showHerbsApp;
        },
        'opened': props.activeApps.showHerbsApp && !props.minimizedApps.HerbsAppMinimized,
      },
      {
        'icon': <TowServiceIcon className={classes.icon}></TowServiceIcon>,
        'title': 'Tow Service',
        'open': function () {
            props.setMinimizedApps(prevState => ({
                ...prevState,
                TowAppMinimized: false
              }));
        },
        'show': function () {
            return props.activeApps.showTowApp;
        },
        'opened': props.activeApps.showTowApp && !props.minimizedApps.TowAppMinimized,
      },
      {
        'icon': <UnknownIcon className={classes.icon}></UnknownIcon>,
        'title': 'Unknown',
        'open': function () {
            return null;
        },
        'show': function () {
            return props.activeApps.showStreetApp;
        },
        'opened': props.activeApps.showStreetApp,
      },
      {
        'icon': <ANALyzerIcon className={classes.icon}></ANALyzerIcon>,
        'title': 'ANALyzer',
        'open': function () {
            props.setMinimizedApps(prevState => ({
                ...prevState,
                SeedAnalyzerAppMinimized: false
              }));
        },
        'show': function () {
            return props.activeApps.showSeedAnalyzerApp;
        },
        'opened': props.activeApps.showSeedAnalyzerApp && !props.minimizedApps.SeedAnalyzerAppMinimized,
      },
      {
        'icon': <DodoIcon className={classes.icon}></DodoIcon>,
        'title': 'Dodo',
        'open': function () {
            props.setMinimizedApps(prevState => ({
                ...prevState,
                DodoAppMinimized: false
              }));
        },
        'show': function () {
            return props.activeApps.showDodoApp;
        },
        'opened': props.activeApps.showDodoApp && !props.minimizedApps.DodoAppMinimized,
      },
      ]

    return (
        <div className={classes.taskbar}>
            
            <div className={classes.mainIcons}>

                <div className={classes.taskbarItem}>
                    <img src="https://cdn.discordapp.com/attachments/1052674807734468748/1198433134224625765/pixel-logo.f199a3e8.png?ex=65bee2d5&is=65ac6dd5&hm=31dc25dd02d02a7397dff7d7faf2cee437c7a232ef6aa6518fc7b4254a30ceb6&" alt="file-icon" className={classes.icon}></img>
                </div>
                <div className={classes.taskbarItem}>
                    <img src="data:image/png;base64," alt="file-icon" className={classes.icon}></img>
                </div>
                <div className={''.concat(classes.taskbarItem, ' ').concat(props.activeApps.showPixelBrowser && classes.open)} onClick={() => 
                            props.setActiveApps(prevState => ({
                                ...prevState,
                                showPixelBrowser: true
                              }))}>
                    <img src="data:image/png;base64," alt="file-icon" className={classes.icon}></img>
                </div>

                {Applications.filter(app => app.show()).map((app, index) => {
                    return (
                        <div className={ app.opened ? ''.concat(classes.taskbarItem, ' ').concat(classes.open) : classes.taskbarItem } key={index}
                        onClick={() => app.open()}>
                            {app.icon}
                        </div>
                    )
                })}
            </div>

            <div className={props.activeApps.showSettingsPanel ? ''.concat(classes.taskbarItem, ' ').concat(classes.open) : classes.taskbarItem} 
                onClick={() => 
                    props.setActiveApps(prevState => ({
                        ...prevState,
                        showSettingsPanel: true
                    }))
                }>
            <FontAwesomeIcon
                        icon={faCog}
                        size="1x"
                        style={{ color: '#fff' }}
                    />
            </div>

            <div className={classes.smallIcon}>
            <FontAwesomeIcon
                        icon={faWifi}
                        size="1x"
                        style={{ color: '#fff' }}
                    />
            </div>

            <div className={classes.systemTime}>
            <Typography
            style={{
                color: '#fff',
                wordBreak: 'break-word',
                fontSize: '1em',
            }}
            variant="body1"
            gutterBottom
            >
            {new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
            </Typography>
            <Typography
            style={{
                color: '#fff',
                wordBreak: 'break-word',
                fontSize: '1em',
            }}
            variant="body1"
            gutterBottom
            >
            {new Date().toLocaleDateString()}
            </Typography>
            </div>
            <div className={props.activeApps.showNotificationPanel ? ''.concat(classes.taskbarItem, ' ').concat(classes.open) : classes.taskbarItem}>
                <div className={classes.notificationIcon} 
                    onClick={() => 
                        props.setActiveApps(prevState => ({
                            ...prevState,
                            showNotificationPanel: true
                        }))
                    }>
                </div>
            </div>

        </div>
    )
}